...
var CommandHandlerMiddleware = function (_Middleware) {
	...
	function CommandHandlerMiddleware(commandNameExtractor, handlerLocator, methodNameInflector) {
		...
		_this.commandNameExtractor = commandNameExtractor;
		_this.handlerLocator = handlerLocator;
		_this.methodNameInflector = methodNameInflector;
		return _this;
	}
	...
	_createClass(CommandHandlerMiddleware, [{
		...
		value: function execute(command, next) {
			...
			if (this.commandNameExtractor instanceof _CommandNameExtractor2.default) {
				commandName = this.commandNameExtractor.extractName(command);
			}

			if (commandName && this.handlerLocator instanceof _HandlerLocator2.default) {
				handler = this.handlerLocator.getHandlerForCommand(commandName);
			}

			if (commandName && handler && this.methodNameInflector instanceof _MethodNameInflector2.default) {
				methodName = this.methodNameInflector.inflect(commandName, handler);
			}

			if ((0, _lodash.isFunction)(handler[methodName])) {
